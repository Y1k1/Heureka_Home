...
# Specify the path to the new file within the script directory
file_path = os.path.join(script_directory, 'src_data_stock_url.json')

# Configuration
username = 'Y1k1'  # Your GitHub username
repo = 'Heureka_Home'  # Repository name
path = 'stock_data_library/src_data_stock_url.json'  # Path where the new file will be placed in the repo
file_name = file_path  # Use the full path of the file

# Your GitHub Personal Access Token - USE ENVIRONMENT VARIABLE OR OTHER SECURE METHODS
pat = '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'  # Replace with a secure method
...
data = {
    'message': 'Upload src_data_stock_url.json',
    'content': content,
    'branch': 'master',  # Replace with your actual branch name
}

# If the file exists, add the SHA to the request data to update it
if sha:
    data['sha'] = sha
