...
# Function to get the last modified times of PNG files in a directory
def get_file_mod_times(folder):
    mod_times = {}
    for filename in os.listdir(folder):
        if filename.endswith(".png"):
            full_path = os.path.join(folder, filename)
            mod_times[filename] = os.path.getmtime(full_path)
    return mod_times

# Initial processing
last_mod_times = get_file_mod_times(input_folder)
for filename in last_mod_times:
...
while True:
    ...
    current_mod_times = get_file_mod_times(input_folder)

    # Check for new or updated files
    updated_files = [f for f in current_mod_times if f not in last_mod_times or current_mod_times[f] > last_mod_times[f]]

    if not updated_files:
        no_change_count += 1
        if no_change_count >= 60:  # 60 seconds of no change
            print("No new or updated files for 60 seconds. Stopping the script.")
            break
    else:
        no_change_count = 0  # Reset counter if there are changes
